...
enum Category {
    ...
    */
    ...
    /* Instruction instance is associated with a  HTML <li> that has a nested, child <ol> that contains sub-instructions.
       ...
       ---------------------------
     */
    VARIPOINTS_OPTION_SET = "VARIPOINTS_OPTION_SET",    
...
enum OptionCategory {
    ...
    CHILD_OF_OPTIONSET = "CHILD_OF_OPTIONSET",    
    /* Instruction instance is of a child of OPTION_SET Instruction  */
    DESCENDENT_OF_OPTIONSET_CHILD = "DESCENDENT_OF_OPTIONSET_CHILD",
}
...
export class Instruction {
    ...
    constructor(s: string = "", n: string = "", sh: string = "", c: Category = Category.GENERAL, pointFraction: number = 0, parent: Instruction = null) {
        ...
        if (this.parent !== null)
        {
            ...
            {/* Override any category that was set in the HTML file, assinging it Category.COMPOSITE
...
export class Instructions 
{
    ...
    extractSectionsAndRubric(
            ...
            topSectionHLevel : number = 1)
        {        
        /**
         *   <section> <h1>
         */
        let h2Fixup : number = (topSectionHLevel===1) ? 0 : 1;//(level > 2 ? 1 :0 ); /* cover switch from using common convention of h2 as top-level <section> number, instead of h1 */
        const selector : string = ":scope section > h" + (level+h2Fixup).toFixed(0);
     ...
     **/
    extractSectionsAndRubricAll(totalPoints:number, topSectionHLevel : number = 1) 
    {        
