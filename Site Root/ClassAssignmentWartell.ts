...
function onLoad_idea2() {
    ...
    for (let instruction of Inst.instructions.instructions) {
        ...
        row.querySelector('input[type="checkbox"]').addEventListener('change',
            (e : InputEvent ) =>
            {
                ...
                (<HTMLInputElement>(<HTMLInputElement>e.currentTarget).parentElement.nextElementSibling.firstChild).value = Inst.instructions.instructions[rowIndex].marks.toString();
            });
        ...
        row.querySelector('input[type="number"]').addEventListener('change',
            (e : InputEvent ) =>
            {
                const rowIndex=parseInt((<HTMLElement>e.srcElement).parentElement.parentElement.getAttribute("data-ri"));
                console.log(Inst.instructions.instructions[rowIndex]);
                Inst.instructions.instructions[rowIndex].marks = parseInt((<HTMLInputElement>e.currentTarget).value);
            });
        ri++;
    }
     ...
     */
    (<HTMLElement>document.querySelector("#Download")).onclick = () => {
